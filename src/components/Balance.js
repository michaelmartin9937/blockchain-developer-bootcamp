...
const Balance = () => {
  ...
  const exchangeBalances = useSelector(state => state.tokens.balances)
  ...
  // Step 1: do deposit (transfer)
  // Step 2: Notify app that deposit (transfer) is pending
  // Step 3: Get confirmation from blockchain that deposit (transfer) was successful
  // Step 4: Notify app that deposit (transfer) was successful

  const depositHandler = (e, token) => {
    e.preventDefault()

    if (token.address === tokens[0].address) {
      // transferTokens(provider, exchange, transferType, token, amount, dispatch)
    ...
    <div className='component exchange__transfers'>
      ...
      <div className='exchange__transfers--form'>
        ...
        <form onSubmit={isDeposit ? (e) => depositHandler(e, tokens[1]) : (e) => withdrawHandler(e, tokens[1])}>
          <label htmlFor="token1">{symbols && symbols[1]}</label>
          <input 
            type="text" 
            id='token1' 
            placeholder='0.0000'
