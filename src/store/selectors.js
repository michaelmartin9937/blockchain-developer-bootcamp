...
export const myOpenOrdersSelector = createSelector(
	...
	(account, tokens, orders) => {
		...
		// Filter order by token addresses
...
const decorateFilledOrders = (orders, tokens) => {
	...
	return (
	  orders.map((order) => {
	    ...
	    previousOrder = order // update the previous order once it's been decorated
	    return order 
	  })
	)
...
export const orderBookSelector = createSelector(
	// the following lines are an explanation of the arguments
	// allOrders - we are passing in all of the orders from the redux store
	// tokens - this is an array that includes token1 and token2
	// third arguent is a callback function - this callback function does things lol
	openOrders, 
	tokens, 
	(orders, tokens) => {
